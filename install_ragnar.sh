...
" && log "INFO" "Config JSON epd_type set to $EPD_VERSION" \
  || log "WARNING" "Could not write shared_config.json — epd_type will be read from shared.py on first run"
    ...
    # GPIO stack for displays. The vendored Waveshare epdconfig.py uses
    # gpiozero (LED/Button pin objects); gpiozero needs a pin factory
    # (lgpio on current Raspberry Pi OS). Missing these shows up as a dead
    # e-Paper panel with no import error in the service log — the driver
    # raises inside epdconfig at first draw. RPi.GPIO remains as a fallback
    # for older driver code paths.
    if [ "$IS_PI" != true ]; then
        log "INFO" "Non-Raspberry Pi host — skipping GPIO display stack (no GPIO displays)"
    else
        # Prefer distro packages: they match the kernel's libgpiod/lgpio ABI.
        local gpio_packages=("python3-gpiozero" "python3-lgpio" "python3-spidev" "python3-rpi.gpio")
        for pkg in "${gpio_packages[@]}"; do
            install_package "$pkg" || log "WARNING" "$pkg not available via $PKG_MGR — trying pip"
        done
        if ! check_python_package "gpiozero"; then
            log "INFO" "Installing gpiozero via pip..."
            pip3 install --break-system-packages gpiozero || \
                log "WARNING" "gpiozero install failed — e-Paper/TFT GPIO displays will not work"
        fi
        if ! check_python_package "lgpio"; then
            log "INFO" "Installing lgpio (gpiozero pin factory) via pip..."
            pip3 install --break-system-packages lgpio || \
                log "WARNING" "lgpio install failed — gpiozero may fall back or fail on Pi 5/Zero 2 W"
        fi
        if ! check_python_package "RPi.GPIO" && ! check_python_package "spidev"; then
            log "INFO" "Installing RPi.GPIO and spidev via pip (fallback)..."
        ...
        if check_python_package "gpiozero"; then
            log "SUCCESS" "GPIO display stack ready (gpiozero + pin factory)"
        else
            log "WARNING" "gpiozero missing — e-Paper/TFT GPIO displays will NOT work"
        fi
    fi
    ...
    if [ "$HEADLESS_MODE" = true ]; then
        ...
        local unusable=()
        python3 -c "import gpiozero" 2>/dev/null || unusable+=("GPIO displays (gpiozero)")
        ...
        # Display smoke test — catch a dead panel at install time instead of
        # after the first reboot. Only touches the SPI bus; if no HAT is
        # attached the init fails fast and we say so, without blocking install.
        local epd_probe="epd2in13_V4"
        if [ -n "${EPD_VERSION:-}" ]; then
            epd_probe="$EPD_VERSION"
        fi
        log "INFO" "Display smoke test ($epd_probe)..."
        if RAGNAR_PATH="${ragnar_PATH:-/home/ragnar/Ragnar}" python3 - "$epd_probe" <<'PY_SMOKE'
import os, sys, warnings
warnings.filterwarnings("ignore")
mod_name = sys.argv[1] if len(sys.argv) > 1 else "epd2in13_V4"
sys.path.insert(0, os.path.join(os.environ.get("RAGNAR_PATH", "."), "resources"))
sys.path.insert(0, "resources")
try:
    import gpiozero  # noqa: F401
except Exception as e:
    print(f"SMOKE_FAIL gpiozero: {e}")
    sys.exit(2)
try:
    import importlib
    ...
    epd = mod.EPD()
    epd.init()
    # Do not clear the panel here — just prove SPI/GPIO handshake works.
    try:
        epd.sleep()
    except Exception:
        pass
    ...
    print(f"SMOKE_FAIL {mod_name}: {type(e).__name__}: {e}")
    sys.exit(1)
PY_SMOKE
        then
            log "SUCCESS" "Display smoke test passed ($epd_probe) — panel responds on SPI/GPIO"
        else
            local smoke_rc=$?
            if [ "$smoke_rc" -eq 2 ]; then
                log "WARNING" "Display smoke test skipped — gpiozero unavailable"
            else
                log "WARNING" "Display smoke test FAILED ($epd_probe) — check HAT seating, SPI enable, or panel revision (V2/V3/V4/B). Install continues."
            fi
        fi
    fi
