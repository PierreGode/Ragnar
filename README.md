...
- **Safe — encrypted file vault** — A password-protected vault in the **Files** tab for storing sensitive files encrypted at rest. On first use you choose how much disk to reserve for it and set a password; every file is then encrypted with **AES-256-GCM** under a key derived from that password with **scrypt** — the key only ever lives in server memory while the Safe is unlocked, and both the file contents *and* the file index are ciphertext on disk. Listing, previewing, downloading and adding files all require unlocking with the password, and the Safe auto-locks after 15 minutes idle (or on demand). **There is no recovery** — forget the password and the files stay encrypted forever. See [Safe Vault](docs/safe.md)
