...
- **Authority Verification across the stack** — is the claimed root bridge / gateway / DNS resolver / DHCP server / routing neighbour / SMB server genuine or an impostor? A network engineer's toolbox (Diagnostics, Switch & L2/L3, Interfaces) plus a **detection-only passive watcher suite spanning L2→L7** (STP/DTP/CDP/VTP/LACP/BFD/PTP/SR-MPLS/IPsec-IKE/DNS-Watch/FHRP/OSPF/EIGRP/IS-IS/BGP/SMB/Relay/RPC/LDAP/SSH/Telnet/IGMP-MLD/NDP/ICMP/NTP/SNMP/TLS/Cert — most dual-stack IPv4+IPv6, with cross-protocol D(HE)at / SWEET32 crypto-attack coverage), a scheduled Network Integrity Monitor, and IPv6 RA Guard. Co-authored by [Solarflere](https://www.instagram.com/solarflere). Full details in the [Authority Verification Guide](docs/nettools.md).
- **Watchtower** — unifies the standalone watcher daemons into one normalized, deduped alert feed with a single push-notification path. See [Watchtower](docs/watchtower.md).
- **Push Notifications** — alerts delivered via **Pushover** and/or **Slack** (incoming webhook). See [Push Notifications](docs/push-notifications.md).
- **Asset Inventory & SIEM forwarding** — turns the flat hosts table into a change-aware inventory (new device / IP move / OUI change / offline, with owner/criticality/authorized tags and rogue-device signatures), lets you permanently **ignore** noisy devices, and ships every alert to syslog/CEF/LEEF, Splunk HEC, Elasticsearch/OpenSearch, or a JSON/Slack webhook. Host liveness unions an `arp-scan` sweep on the LAN interface (wired or wireless) with the kernel neighbour table, so a single missed broadcast never flaps the whole target list Offline. See [Asset Inventory](docs/asset-inventory.md) and [SIEM Forwarding](docs/siem.md).
...
- **Sub-GHz RF instrument (RTL-SDR / HackRF)** — a real spectrum analyser in the browser: real-time IQ waterfall with adjustable display range, wheel/drag zoom, markers with Δ and peak search, band-plan labels and signal identification, dBm calibration, noise-print subtraction, limit lines with pass/fail alerts into Watchtower, unattended band surveys with reports, RSSI direction finding across mesh units, mesh/LPWAN band overlays (Z-Wave, Meshtastic, MeshCore, LoRaWAN, Wi-Fi HaLow, Zigbee Suzi), CSV/SigMF export and a listen bar (FM/NFM/AM/SSB/CW). See [RF Waterfall](docs/rf-waterfall.md), the [capability guide](docs/rf-waterfall-guide.md) and [SDR / Sub-GHz](docs/sdr-subghz.md). Detectors (peak/RMS/avg), the RTL-SDR centre spike hidden by offset tuning, overload and image checks, mask-triggered capture with a pre-trigger buffer, ACPR/spur/harmonic measurements, geotagged SigMF recordings, and the whole instrument is scriptable over HTTP ([API](docs/rf-api.md)). A self-healing watcher recovers a stuck or silent dongle by power-cycling its USB port and resuming the sweep, and says so plainly when the dongle, cable or port is at fault.
- **WiFi Defense (802.11 WIDS)** — passive wireless intrusion detection: deauth/disassoc & beacon floods, rogue APs / evil twins, KARMA/MANA, Wi-Fi Pineapple / PineAP-family rogue APs (Mark VII/Enterprise/Pager), and a client-isolation observer, with a printable WIDS incident report. The continuous monitor is receive-only; an opt-in PineAP active-probe test is the one action that transmits, and only when you run it. Its daemon sibling [wifiwatch](docs/wifiwatch.md) extends this to the WPA handshake layer (PMKID, downgrade, PNL leakage). See [WiFi Defense](docs/wifi-defense.md).
- **Wardriving with GPS recovery** — logs WiFi/BLE/cell with GPS positions, exports WiGLE CSV / KML and a printable A–F security-graded survey, uploads sessions (manually or automatically once back online) to WiGLE, WDGWars and [Wardrift](https://wardrift.net), and backfills missing positions with speed-aware interpolation. Includes a deep [Diagnostics panel](docs/diagnostics.md) (radios/power/GPS sky view + [Starview observatory](docs/diagnostics.md#ragnar-starview--the-observatory-mode)), [cellular modem](docs/cell.md) capture, and a [power badge](docs/power.md) — the System tab adds a load **power test** and a one-click fix for the Pi 5's 600 mA USB current cap (also applied by the installer on Pi 5). See [Wardriving](docs/wardriving.md).
