...
- **Home Assistant integration** — A [HACS](https://hacs.xyz/) custom integration (shipped in-repo under `custom_components/ragnar/`) that surfaces a unit's **RuSense presence / people-count / vitals** and **Watchtower security alerts + incidents** as native Home Assistant entities — occupancy and safety `binary_sensor`s, `sensor`s for heart/breathing rate and alert/incident/vulnerability counts, and an `event` entity that fires per new security alert — so Ragnar can drive HA automations (presence-triggered lighting, phone push on an evil-twin alert) and dashboards. It is a read-only **local-polling** client against Ragnar's existing web API; auth reuses Ragnar's session-cookie login (or none, on an open unit). A config-flow UI handles setup. See [Home Assistant integration](docs/homeassistant.md)
