...
- **Sub-GHz RF instrument (RTL-SDR / HackRF)** — a real spectrum analyser in the browser: real-time IQ waterfall with adjustable display range, wheel/drag zoom, markers with Δ and peak search, band-plan labels and signal identification, dBm calibration, noise-print subtraction, limit lines with pass/fail alerts into Watchtower, unattended band surveys with reports, RSSI direction finding across mesh units, CSV/SigMF export and a listen bar (FM/NFM/AM/SSB/CW). See [RF Waterfall](docs/rf-waterfall.md), the [capability guide](docs/rf-waterfall-guide.md) and [SDR / Sub-GHz](docs/sdr-subghz.md). Detectors (peak/RMS/avg), overload and image checks, mask-triggered capture with a pre-trigger buffer, ACPR/spur/harmonic measurements, geotagged SigMF recordings, and the whole instrument is scriptable over HTTP ([API](docs/rf-api.md)).
