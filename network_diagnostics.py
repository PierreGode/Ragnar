...
def register_network_diagnostics(app, logger=None):
    ...
    # VOR radial decode (108-118 MHz nav beacon) — recovers the bearing FROM the
    # station from its two 30 Hz tones. Uses the whole RTL-SDR (rtl_fm -M am).
    @app.route('/api/net/vor/status', methods=['GET'])
    def net_vor_status():
        return jsonify(vor.status())

    @app.route('/api/net/vor/start', methods=['POST'])
    def net_vor_start():
        data = request.get_json(silent=True) or {}
    ...
    @app.route('/api/net/vor/stop', methods=['POST'])
    def net_vor_stop():
        _log("net/vor/stop")
        return jsonify(vor.stop())
    ...
    @app.route('/api/net/acars/status', methods=['GET'])
    def net_acars_status():
        return jsonify(acars.status())

    @app.route('/api/net/acars/messages', methods=['GET'])
    def net_acars_messages():
        return jsonify(acars.messages(since=request.args.get('since', 0)))

    @app.route('/api/net/acars/start', methods=['POST'])
    def net_acars_start():
    ...
    @app.route('/api/net/acars/stop', methods=['POST'])
    def net_acars_stop():
        _log("net/acars/stop")
        return jsonify(acars.stop())

    @app.route('/api/net/acars/install', methods=['POST'])
    def net_acars_install():
        _log("net/acars/install")
        return jsonify(acars.install())
