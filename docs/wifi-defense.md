...
| **Attack-tool SSID** | An AP beaconing under a **known ESP32/handheld attack-tool or captive-portal default name** — `EvilPortal` (ESP32 Marauder / Flipper EvilPortal), `Marauder`, `Ghost ESP`, `GARMR`, `Bruce`, `WiFi Pineapple`, `Pwnagotchi`, `Deauther`, … | The SSID, normalized (lowercased, spaces/`_`/`-` stripped so `Evil Portal` = `evil_portal` = `EvilPortal`), matches a curated attack-tool name list. These names are essentially never legitimate, so this is a **critical** flag on its own — it catches a lone captive-portal AP the behavioural checks can't see without a real AP to clone. Open **or** secured. Carries a **"Probe captive portal…"** button. |
...
|--------|---------|--------|
| **Wi-Fi** | **attack-tool SSID** (a known EvilPortal/Marauder/Ghost ESP/GARMR default name — high-confidence), auth flood, evil-twin, **spoofed BSSID** (group/multicast I/G bit set — no real AP does this), **open rogue-lure** (an open AP using a common free-Wi-Fi name — low-confidence), beacon flood, KARMA, deauth — tool-agnostic behaviour | this WIDS capture |
